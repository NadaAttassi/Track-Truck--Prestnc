...
const moroccanCitiesDb = [
  ...
  { name: "Tanger", lat: 35.7595, lon: -5.834, radius: 0.3 },
  ...
  { name: "Kénitra", lat: 34.261, lon: -6.5802, radius: 0.3 },
  ...
  { name: "Errachidia", lat: 31.9316, lon: -4.423, radius: 0.2 },
  ...
  { name: "Tan-Tan", lat: 28.438, lon: -11.1031, radius: 0.2 },
...
async function aStar(startLat, startLon, endLat, endLon) {
  ...
  for (const nodeKey of graph.keys()) {
    gScore.set(nodeKey, Number.POSITIVE_INFINITY)
    fScore.set(nodeKey, Number.POSITIVE_INFINITY)
  }
  ...
  while (!openSet.isEmpty()) {
    ...
    for (const neighbor of neighbors) {
      ...
      if (tentativeGScore < gScore.get(neighborKey)) {
        ...
        fScore.set(
          neighborKey,
          tentativeGScore + calculateDistance(...neighborKey.split(",").map(Number), endLat, endLon),
        )
