...
types.setTypeParser(1700, (value) => Number.parseFloat(value)) // NUMERIC type
...
const moroccanCitiesDb = [
  ...
  { name: "Tanger", lat: 35.7595, lon: -5.834, radius: 0.2 },
  ...
  { name: "Kénitra", lat: 34.261, lon: -6.5802, radius: 0.2 },
  { name: "Tétouan", lat: 35.5784, lon: -5.3622, radius: 0.2 },
  { name: "Safi", lat: 32.2994, lon: -9.2372, radius: 0.2 },
  { name: "Mohammedia", lat: 33.6864, lon: -7.383, radius: 0.2 },
  { name: "El Jadida", lat: 33.2549, lon: -8.5079, radius: 0.2 },
  { name: "Béni Mellal", lat: 32.3367, lon: -6.3606, radius: 0.2 },
  { name: "Nador", lat: 35.1681, lon: -2.933, radius: 0.2 },
  { name: "Essaouira", lat: 31.5125, lon: -9.77, radius: 0.2 },
  { name: "Taza", lat: 34.21, lon: -4.01, radius: 0.2 },
  { name: "Khouribga", lat: 32.8811, lon: -6.9063, radius: 0.2 },
  { name: "Ouarzazate", lat: 30.92, lon: -6.8936, radius: 0.3 },
  { name: "Settat", lat: 33.0014, lon: -7.617, radius: 0.2 },
  ...
  { name: "Tan-Tan", lat: 28.438, lon: -11.1031, radius: 0.25 },
...
function isInMorocco(element, city) {
  ...
  const isInMoroccanCoordinates =
    element.lat >= 27.5 && element.lat <= 36.0 && element.lon >= -13.2 && element.lon <= -1.0

  // Exclure Ceuta et Melilla
  const isCeuta = element.lat >= 35.8 && element.lat <= 36.0 && element.lon >= -5.4 && element.lon <= -5.2

  const isMelilla = element.lat >= 35.2 && element.lat <= 35.4 && element.lon >= -3.1 && element.lon <= -2.9
...
function determineCity(element) {
  ...
  for (const city of moroccanCitiesDb) {
    const distance = Math.sqrt(Math.pow(element.lat - city.lat, 2) + Math.pow(element.lon - city.lon, 2))
  ...
  for (const region of moroccanCitiesDb.slice(-7)) {
    // Les 7 derniers éléments sont les régions
    const distance = Math.sqrt(Math.pow(element.lat - region.lat, 2) + Math.pow(element.lon - region.lon, 2))
...
async function insertPlaces(places) {
  ...
  try {
    ...
    const params = places.flatMap((place) => [place.name, place.lat, place.lon, place.id, place.type, place.city])
...
async function fetchPlaces() {
  ...
  try {
    ...
    const batchSize = 500 // Insérer par lots de 500
    let placeBatch = []
...
async function main() {
  ...
  try {
    ...
    const topCities = await client.query(
      "SELECT city, COUNT(*) as count FROM places GROUP BY city ORDER BY count DESC LIMIT 10",
    )
    ...
    const cityRegions = await client.query(
      "SELECT city, COUNT(*) as count FROM places WHERE city LIKE 'Région%' GROUP BY city ORDER BY count DESC",
    )
